...
from GTEPdata_SDDP_10years import *
import itertools
...
print("before node filter")
n_stage = [(t,n) for t in m.t for n in N_stage[t]]
# print(n_stage)
m.n_stage= Set(initialize=n_stage)
print("after node filter")
...
m.L_max = Param(m.n_stage, default=0, initialize=L_max)
# m.L_max = Param(m.t, default=0, initialize=L_max)
m.cf = Param(m.i, m.r, m.t, m.d, m.s, default=0, initialize=cf)
m.Qg_np = Param(m.i_r, default=0, initialize=Qg_np)
m.Ng_max = Param(m.i_r, default=0, initialize=Ng_max)
...
m.hr = Param(m.i_r, default=0, initialize=hr)
...
def planning_block_rule(b,t,n):
    ...
    def obj_rule(_b):
        return m.if_[t]*\
        (sum(m.n_d[d]*m.hs* \
            	sum((m.VOC[i,t] + m.hr[i,r]*m.P_fuel[i,t]#,n]
