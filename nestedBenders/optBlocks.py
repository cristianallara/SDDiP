...
m.t = RangeSet(SDDiP.time_periods)

# Superset of iterations
m.iter = RangeSet(SDDiP.max_iter)

# Set of iterations for which cuts are generated
m.k = Set(within=m.iter, dimen=1, ordered=True)

t_stage = [(t, n) for t in m.t for n in SDDiP.n_stage[t]]
print(t_stage)
...
m.prob = Param(SDDiP.nodes, initialize=SDDiP.prob)
