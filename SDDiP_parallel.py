...
with pymp.Parallel(NumProcesses) as p:
    ...
    m.nso_par = Param(m.j, m.r, m.t_node, default=0, initialize=0, mutable=True)
    m.nsb_par = Param(m.j, m.r, m.t_node, default=0, initialize=0, mutable=True)
        ...
        for n in n_stage_processid[pid][stage]:
            if stage != 1:
                ...
                for (j, r) in j_r:
                    for pn in n_stage_processid[pid][stage - 1]:
                        if pn in parent_node[n]:
                            m.Bl[stage, n].link_equal3.add(expr=(m.Bl[stage, n].nso_prev[j, r] ==
                                                                 m.nso_par[j, r, t_prev, pn]))
